...
const anyProviderObserver = async () => {
  try {
...
const walletChoosingObserver = async () => {
  try {
    ...
    accountsChangeObservable.subscribe((accounts) => {
      mixpanel.track("ACCOUNT_CONNECTED", {
        account: accounts[0],
      });
      setPersonalStatisticsEvents(providerWrapper, contract, accounts[0]);
    });
    const requestAccountObservable = from(requestAccounts(provider)).pipe(
      takeUntil(accountsChangeObservable)
    );
    requestAccountObservable.subscribe((accounts) =>
      setPersonalStatisticsEvents(providerWrapper, contract, accounts[0])
    );
    setGlobalStatisticsEvents(contract);
  } catch (Exception) {
